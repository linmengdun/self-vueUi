  ...
  <div class="file-diff-view">
    <div class="toolbar">
      ...
      <transition name="vue-ui-fade">
        <VueLoadingIndicator v-if="loading && fileDiffs.length" class="small accent" />
      </transition>
      <div class="vue-ui-spacer" />
      <VueInput v-model="search" icon-left="search" :placeholder="$t('org.vue.components.file-diff-view.search-file')" />
      <VueButton :icon-left="allCollapsed ? 'keyboard_arrow_down' : 'keyboard_arrow_up'"
    ...
    <div class="list">
      ...
      <template v-else>
        <FileDiff v-for="fileDiff of filteredList" :key="fileDiff.id" :file-diff="fileDiff"
          :collapsed="!!collapsed[fileDiff.id]" @update:collapsed="value => $set(collapsed, fileDiff.id, value)" />
      </template>
    </div>
    <div class="actions-bar">
      <template v-if="!error && fileDiffs && fileDiffs.length">
        <VueButton icon-left="vertical_align_bottom" :label="$t('org.vue.components.file-diff-view.actions.commit')"
          class="big primary" @click="showCommitModal = true" />
        <VueButton :label="$t('org.vue.components.file-diff-view.actions.skip')" class="big" data-testid="skip-button"
          @click="skip()" />
      </template>
      <template v-else>
        <VueButton icon-left="done" :label="$t('org.vue.components.file-diff-view.actions.continue')" class="big primary"
          data-testid="skip-button" @click="skip()" />
        <VueButton :label="$t('org.vue.components.file-diff-view.actions.refresh')" icon-left="refresh" class="big"
          @click="refresh()" />
      </template>
    </div>

    <transition name="vue-ui-fade">
      <VueLoadingIndicator v-if="loading && !fileDiffs.length" class="accent big overlay" />
    </transition>

    <VueModal v-if="showCommitModal" :title="$t('org.vue.components.file-diff-view.modals.commit.title')" class="medium"
      @close="showCommitModal = false">
      <div class="default-body">
        <VueFormField :title="$t('org.vue.components.file-diff-view.modals.commit.input')"
          :subtitle="$t('org.vue.components.file-diff-view.modals.commit.subtitle')">
          <VueInput v-model="commitMessage" icon-left="local_offer" v-focus @keyup.enter="commitMessage && commit()" />
        </VueFormField>
      </div>

      <div slot="footer" class="actions center">
        <VueButton :label="$t('org.vue.components.file-diff-view.modals.commit.actions.cancel')" class="flat"
          @click="showCommitModal = false" />
        <VueButton :label="$t('org.vue.components.file-diff-view.modals.commit.actions.commit')" class="primary"
          icon-left="vertical_align_bottom" :disabled="!commitMessage" @click="commit()" />
      </div>
    </VueModal>
  </div>
</template>
